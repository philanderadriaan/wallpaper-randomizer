...
public final class PixelImage extends BufferedImage {
	...
	public static PixelImage load(final File the_file) throws IOException {
		...
		final PixelImage image = new PixelImage(buf.getWidth(), buf.getHeight(), BufferedImage.TYPE_INT_RGB);
	 ...
	 */
	public void save(final File the_file, String ext) throws IOException {
		ImageIO.write(this, ext, the_file);
	}

	/**
	 * Return the image's pixel data as an array of Pixels. The first coordinate is
	 * the y-coordinate, so the size of the array is [height][width], where width
	 ...
	 *                                     image dimensions or has invalid pixel
	 *                                     values.
	 */
